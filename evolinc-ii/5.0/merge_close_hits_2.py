            ...
            if line_id not in ids:
                ...
                if line_id in ids:
                    ...
                    for entry in line_array:
                        entry = entry.strip('\n')
                        new_line_array.append(entry)
                    new_line_array.append('_Pass')
                    new_line = '\t'.join(new_line_array) + '\n'
                    otpt.write(new_line)

sorting_command = 'sort -k1,1 -k2,2 -k11,11r -t"\t" --output="%s" %s' % (input_file_sorted, input_file_sorted)
        ...
        for line in inpt:
            ...
            #Process data-containing lines
                ...
                if line_array[0] == line_memory_array[0]:
                    ...
                    hsp_length = abs(stop - start)
                    #Combine HSPs (if appropriate)
                    if int(line_array[9]) + 1000 >= hsp_length > 0 and line_array[1] == line_memory_array[1]:
                        #Add top blast hit to memory array, if appropriate
                        try:
                            if line_array[10] == '_TBH':
                                line_memory_array[0] += line_memory_array[10]
                                line_array[0] += line_array[10]
                                printline = "\t".join(line_memory_array) + '\n'
                                otpt.write(printline)
                            else:
                                if line_memory_array[10] == '_TBH':
                                    line_array[10] = line_memory_array[10]
                                    line_array[0] += line_array[10]
                                    printline = "\t".join(line_array) + '\n'
                                    otpt.write(printline)
                                else:
                                    pass
                                pass
...
call('rm merge_close_hits_input_sorted.txt', shell=True)
call('rm sample-instance-report.txt', shell=True)
call('rm scriptpy-temp-out.txt', shell=True)
