...
sh /evolinc_docker/Collapse_GTF_Entries.sh -c filtered.lincRNA.gtf -o filtered.lincRNA.collapsed.gtf
gff2bed < filtered.lincRNA.collapsed.gtf > lincRNA.prefilter.bed
...
# Modifying the renaming list
awk '{$1=$1}1' OFS="|" renaming_list.txt | sed -e 's/^/s|/g' -e 's/$/|/' > temp && mv temp renaming_list.txt
sed -i.bak -f renaming_list.txt lincRNA.updated.gtf
rm ../comparefile.gtf
