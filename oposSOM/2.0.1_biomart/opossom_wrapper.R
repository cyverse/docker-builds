...
# Biomart text file
bio <- read.csv("/biomart_files.txt", sep = "\t")
full <- bio[grep(ret.opts$species, bio$full_name),]

databasebiomart <- bio %>% filter(full_name == ret.opts$species) %>% pull(Biomart) %>% as.character()
databasehost <- bio %>% filter(full_name == ret.opts$species) %>% pull(Host) %>% as.character()
databasedataset <- bio %>% filter(full_name == ret.opts$species) %>% pull(dataset) %>% as.character()
databaseidtype <- "ensembl_gene_id"
datasetname <- ret.opts$species
